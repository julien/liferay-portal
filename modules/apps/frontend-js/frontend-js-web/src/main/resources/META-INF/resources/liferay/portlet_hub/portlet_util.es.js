 ...
 *
 * @param  {Object} renderState  The page render state.
 * @param  {string} updateString The update string to decode.
 * @return {Object}
 * @review
 */

const decodeUpdateString = function(renderState, updateString) {
	const portlets = {};

	try {
		const pageRenderState = JSON.parse(updateString);
		const portlets = renderState.portlets;

		if (pageRenderState.portlets && portlets) {
			const keys = Object.keys(portlets);

			for (const key of keys) {
				const newState = pageRenderState.portlets[key].state;
				...
				if (stateChanged(newState, key)) {
					portlets[key] = pageRenderState.portlets[key];
				}
			}
		}
...
const	generateParameterString = function(renderState, portletId, name, type, group) {
	let str = '';

	const portletData = renderState.portlets[portletId];
...
const generatePortletModeAndWindowStateString = function(renderState, portletId) {
	let str = '';
...
const getUpdatedPublicRenderParameters = function(renderState, portletId, state) {
	const portletData = renderState.portlets[portletId];
	...
	if (portletData && portletData.pubParams) {
		...
		for (let key of keys) {
			if (!isParameterInStateEqual(renderState, portletId, state, key)) {
 ...
 *
 * @param {Object} renderState The global render state
...
const getUrl = function(renderState, type, portletId, parameters, cache, resourceId) {
	...
	if (type === 'RESOURCE') {
		url = decodeURIComponent(renderState.portlets[portletId].encodedResourceURL);
		...
		url = decodeURIComponent(renderState.portlets[portletId].encodedRenderURL);
	}
	else if (type === 'RENDER') {
		url = decodeURIComponent(renderState.encodedCurrentURL);
	}
	else if (type === 'ACTION') {
		url = decodeURIComponent(renderState.portlets[portletId].encodedActionURL);
	...
	if (type !== 'RESOURCE' || cacheability !== 'cacheLevelFull') {
		...
		if (portletId) {
			...
			if (portletData.state && portletData.state.parameters) {
				...
				for (let key in keys) {
					if (!isPublicParameter(renderState, portletId, key)) {
						str += generateParameterString(renderState, portletId, key, RENDER_PARAM);
					}
				}
				url += str;
			}
		...
		if (renderState.prpMap) {
			const publicRenderParameters = {};

			let str = '';

			const mapKeys = Object.keys(renderState.prpMap);
			for (let mapKey of mapKeys) {
				const groupKeys = Object.keys(renderState.prpMap[mapKey]);
				for (let groupKey of groupKeys) {
					const groupName = renderState.prpMap[mapKey][groupKey];
...
const isParameterInStateEqual = function(renderState, portletId, state, name) {
	let result = false;
...
const isPublicParameter = function(renderState, portletId, name) {
	const portletData = renderState.portlets[portletId];
 ...
 *
 * @param   {Object}      renderState    The current page render state.
...
const stateChanged = function(renderState, newState, portletId) {
	let result = false;

	const portletData = renderState.portlets[portletId];

	if (portletData) {
		const oldState = renderState.portlets[portletId].state;
